#!/usr/bin/env python3
"""
Direct runner for dispatching rules evaluation that avoids argparse conflicts.
"""
...
# Store arguments before any imports that might trigger Params.py
stored_argv = sys.argv[:]
sys.argv = ['evaluate_dispatching_rules.py']  # Reset argv to avoid Params.py conflicts

# Now safe to import
from JSSP_Env import SJSSP
from dispatching_rules import DispatchingRules, apply_dispatching_rule

# Restore arguments for our use
max_instances = None
dataset_type = 'all'

for i, arg in enumerate(stored_argv):
    if arg == '--max_instances' and i + 1 < len(stored_argv):
        max_instances = int(stored_argv[i + 1])
    elif arg == '--dataset_type' and i + 1 < len(stored_argv):
        dataset_type = stored_argv[i + 1]
    elif arg in ['-h', '--help']:
        print("Usage: python3 run_dispatching_eval.py [--max_instances N] [--dataset_type all|benchmark|generated]")
        sys.exit(0)
